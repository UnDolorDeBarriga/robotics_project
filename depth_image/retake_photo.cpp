...
int main(int argc, char *argv[]) {
    if (argc != 7) {
        printf("Usage: %s <number of images total> <minimum distance(mm)> <maximum distance(mm)> <number of frames> <cell discretization(mm)> <image to retake>\n", argv[0]);
        return EXIT_FAILURE;
    }
